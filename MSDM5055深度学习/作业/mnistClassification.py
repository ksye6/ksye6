...
class Softmax(Node):
    ...
    def backward(self, delta):
        ...
        return delta * self.y - np.sum(delta * self.y) * self.y
        # return np.dot(delta, (np.diag(self.y) - np.outer(self.y, self.y)))
...
class CrossEntropy(Node):
    ...
    def forward(self, x, l):
        ...
        self.l = l
        self.x = x
        return -np.sum((np.log(x + 1e-6) * l))

    def backward(self, delta):
        '''
        Implement this part
        '''
        return - self.l / (self.x + 1e-6)
...
def load_MNIST():
    ...
    for obj in objects:
        if not os.path.isfile(path + obj):
            cmd = ["curl", "-o", path + obj + end, base + obj + end]
            subprocess.check_call(cmd)
            with gzip.open(path + obj + end, 'rb') as gz_file:
                 with open(path + obj, 'wb') as out_file:
...
if __name__ == '__main__':
    ...
    net = [Linear(dim_img, hidden1),Sigmoid(), Linear(hidden1, num_digit),Softmax(), CrossEntropy(), Mean()]
